...
class _EditMedicineScreenState extends State<EditMedicineScreen> {
  ...
  void initState() {
    super.initState();
    _namaObatController = TextEditingController(text: widget.medicine.namaObat);
    _dosisController = TextEditingController(text: widget.medicine.dosis);
    _catatanController = TextEditingController(text: widget.medicine.catatan);

    // Load existing schedules for this medicine
    _loadExistingSchedules();
  }

  // Load existing schedules and populate UI
  Future<void> _loadExistingSchedules() async {
    print(
        '📋 Loading existing schedules for medicine ID: ${widget.medicine.id}');

    final schedules =
        await _apiService.getSchedulesByMedicineId(widget.medicine.id!);

    if (schedules.isEmpty) {
      print('⚠️ No schedules found for this medicine');
      return;
    }

    setState(() {
      // Extract unique times from schedules
      final timeStrings = schedules.map((s) => s.waktuMinum).toSet().toList();
      _selectedTimes.clear();

      for (var timeStr in timeStrings) {
      ...
      print(
          '✅ Loaded ${_selectedTimes.length} times: ${_selectedTimes.map((t) => t.format(context)).join(", ")}');

      // Determine recurrence type from schedules
      // Count unique dates to determine type
      final uniqueDates = schedules.map((s) => s.tanggal).toSet();

      if (uniqueDates.length == 1) {
        // Single date = 'once'
        _recurrenceType = 'once';
        print('🔄 Recurrence type: once');
      } else {
        // Check if it's daily or weekly by analyzing date patterns
        final sortedDates =
            schedules.map((s) => DateTime.parse(s.tanggal)).toList();
        sortedDates.sort();

        if (sortedDates.length >= 2) {
          // Check if consecutive days (daily) or pattern matches weekly
          bool isDaily = true;
          ...
          if (isDaily) {
            ...
            // Calculate duration
            _recurrenceDays = sortedDates.length ~/ _selectedTimes.length;
            print('🔄 Recurrence type: daily ($_recurrenceDays days)');
          } else {
            // Assume weekly
            _recurrenceType = 'weekly';

            // Extract days of week from schedules
            _selectedDays.clear();
            final dayNumbers = <int>{};

            for (var schedule in schedules) {
              final date = DateTime.parse(schedule.tanggal);
              // Convert to day index: Monday=0, Sunday=6
              int dayIndex = (date.weekday - 1) % 7; // Monday is 1 in Dart
              dayNumbers.add(dayIndex);
            }

            // Convert day indices back to Indonesian day names
            for (var dayIndex in dayNumbers.toList()..sort()) {
              _selectedDays.add(_daysOfWeek[dayIndex]);
            }

            print('🔄 Recurrence type: weekly');
  ...
  Future<void> _updateData() async {
    ...
    if (!success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Gagal update obat'),
    ...
    // Simpan jadwal dengan recurrence menggunakan API
    ...
    print('🗑️ Deleting old schedules for medicine ID: ${widget.medicine.id}');
    await _apiService.deleteSchedulesByMedicineId(widget.medicine.id!);
    ...
    for (var time in _selectedTimes) {
      ...
      print(
          '📤 Sending: medicineId=${widget.medicine.id}, time=$waktuMinum, repeatType=$_recurrenceType, repeatDays=$repeatDays');
      ...
      if (response['success'] == true) {
        ...
        if (jadwalCount > 0) {
          ...
          print('✅ Schedule creation successful for this time');
        } else if (jadwalCount == 0 && response['note'] != null) {
          // Duplicate schedule - already exists
          print('ℹ️ Schedule already exists (${response['note']})');
          print('✅ Skipped duplicate - treating as success');
          totalJadwalDibuat += 1; // Count as 1 to mark success
        } else {
          print('✅ Success! Added $jadwalCount schedules');
          print('✅ Schedule creation successful for this time');
        }
    ...
    if (totalJadwalDibuat > 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              '✅ Obat berhasil diupdate dengan $totalJadwalDibuat jadwal\n'
              '$timeInfo'
              '$scheduleInfo',
